...
作業（集計・計算・抽出など）のために一時的なスクリプトを書いて実行するのは構わない。ただし `scripts/` 配下のファイルや `*.cjs` / `*.js` などのツール類を **コミットに含めてはならない**。`scripts/` は Constitution 第3条で不可変（人間のみ改変可）であり、含めると constitution-guard が PR 全体をブロックする。コミットしてよいのは上記「改変してよいファイル」に明記された出力のみ。`git add` の前に `git status` で意図しないファイル（ヘルパースクリプト・scratch ファイル）が含まれていないか必ず確認すること。
